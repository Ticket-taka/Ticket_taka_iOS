...
final class TeamViewController: BaseViewController {
    ...
    private func presentTeamSpaceiewController(indexPath: IndexPath) {
        // FIXME: - 각 셀에 맞는 데이터 주입 ex. TeamId
        let viewController = TeamSpaceViewController()
        // viewController.hidesBottomBarWhenPushed = true
        navigationController?.pushViewController(viewController, animated: true)
    }
}
...
extension TeamViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        presentTeamSpaceiewController(indexPath: indexPath)
    }
}
